    ...
    <section class="max-w-7xl mx-auto px-2 py-6">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        <div 
          v-for="book in books" 
          :key="book.id" 
          class="bg-[#FFF8EC] rounded-2xl border-2.5 border-black shadow-[4px_4px_0px_#1A1A1A] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all overflow-hidden flex flex-col justify-between"
        >
          <div>
            ...
            <div class="p-5">
              ...
              <p class="text-xs font-bold text-slate-500 mb-3">Terbit: {{ book.tahun_terbit || '-' }}</p>
          ...
          <div class="p-5 pt-0 flex items-center justify-between border-t-2 border-black pt-4">
            ...
            <button 
              ...
              class="px-4 py-2 rounded-xl bg-[#C8F53F] text-black border-2 border-black font-black text-xs shadow-[2px_2px_0px_#1A1A1A] hover:translate-x-[-1px] hover:translate-y-[-1px] active:translate-x-[1px] active:translate-y-[1px] transition-all flex items-center gap-1.5"
            >
              <span>🛒</span> Tambah
            </button>
          </div>
